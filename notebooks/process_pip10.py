...
if __name__ == '__main__':
    ...
    SPECIFIC_EXPERIMENT = 5
    N_CLUSTERS = 20
    T = 2000

    processed_path = all_exps[SPECIFIC_EXPERIMENT]
    all_nii = glob.glob(processed_path + '/motion_corrected_*.nii')
    print(f'for experiment {processed_path}, there are {len(all_nii)} nii files')
    ...
    for i in trange(brain_array.shape[-1]):
        t0 = time.time()
        single_nii = f'{processed_path}/motion_corrected_volume{i}.nii'
        ...
        del data
        del file
    logger.info("Brain Array Assignment Complete")

    logger.info("Starting zscoredF calculation")
    t0 = time.time()
    df = calculate_zscoredF_voxels(brain_array)
    # print(f'{time.time() - t0}s to calculate zscoredF voxels')
    ...
    hf = h5py.File(f'{processed_path}/{N_CLUSTERS}_signals_260713.h5', 'w')
